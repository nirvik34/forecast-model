...
def predict(date: str):
    sel_date = pd.to_datetime(date)
    results = []

    for problem in problem_types:
        df_sub = df[df['Sub Category'] == problem]
        if df_sub.empty:
            results.append({"problem": problem, "predicted_complaints": None, "error": "No data"})
            continue
        ...
        if len(X_seq) < MIN_SAMPLES:
            results.append({"problem": problem, "predicted_complaints": None, "error": "Not enough data"})
            continue
        ...
        model.fit(X_train_scaled, y_train_scaled, epochs=30, batch_size=8, verbose=0, callbacks=[es])

        start_date = daily['date'].iloc[-1]
        days_ahead = (sel_date - start_date).days
        if days_ahead <= 0:
            results.append({"problem": problem, "predicted_complaints": None, "error": "Date is in dataset range"})
            continue
    ...
    total = sum(r["predicted_complaints"] for r in results if r["predicted_complaints"] is not None)
    for r in results:
        if r["predicted_complaints"] is not None and total > 0:
            r["percentage"] = round((r["predicted_complaints"] / total) * 100, 2)

    return {"date": str(sel_date.date()), "results": results}
